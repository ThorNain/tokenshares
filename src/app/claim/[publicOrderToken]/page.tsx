/**
 * Page d'atterrissage du QR code (/claim/{token}) — section 7.
 * N'affiche AUCUNE donnée sensible sans authentification : uniquement des
 * informations génériques sur la commande. Le propriétaire authentifié est
 * redirigé vers son espace après vérification côté serveur.
 */
...
export default async function ClaimPage({ params }: { params: { publicOrderToken: string } }) {
  const qr = await prisma.qrCode.findUnique({
    where: { publicToken: params.publicOrderToken },
  ...
  if (!qr.active) {
    return (
      <Shell>
        ...
        <LoginCta token={params.publicOrderToken} />
      </Shell>
    );
  }
...
function LoginCta({ token }: { token: string }) {
  return (
